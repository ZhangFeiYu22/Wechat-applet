<template>
  <div class="my" @click.stop="hideZanAndPinglun">
    ...
    <div class="head">
      <div class="headImg">
        <img :src="memberInfo.avatar" mode="aspectFill" />
        <div class="hierarchy">
          <div class="nameSex">
            <p class="name">{{ memberInfo.nickName }}</p>
            <p class="sex">
              <i class="iconfont icon-nan" v-if="memberInfo.gender == 1"></i>
              <i
                class="iconfont icon-nv"
                v-else-if="memberInfo.gender == 2"
              ></i>
    ...
    <div class="baseInfo">
      <div class="nameSexPhone">
        ...
        <p class="sex">
          <i
            class="iconfont"
            :class="memberInfo.gender == 1 ? 'icon-nan' : 'icon-nv'"
          ></i>
        </p> -->
        <p class="phone">{{ memberInfo.mobile }}</p>
        <div class="editInfoBtn">
          <div class="sixin" @click="sixinFun">
            <i class="iconfont icon-sixin"></i>
            <span>私信</span>
          </div>
          <div
            class="rightIcon"
            @click="isAttenToggle(memberInfo.isFollow, memberInfo.id)"
          >
            <p class="attention attentioned" v-if="memberInfo.isFollow">
              已关注
            </p>
      ...
      <div class="other">
        ...
        <p class="follower">
          {{
            memberInfo.followCount > 0
              ? memberInfo.followCount + "人关注"
              : "暂无关注"
          }}
        </p>
        <p class="autonym">{{ memberInfo.isAuth ? "已实名" : "未实名" }}</p>
      </div>
    </div>
    <div class="linew"></div>
    ...
    <div class="contentList w94" v-if="navType == '0'">
      <div class="lengthNo" v-if="forumList.length == 0">
        该用户暂无话题动态
      </div>
      <div class="contentItem" v-for="(item, index) in forumList" :key="index">
        <div class="content" :class="item.showEllip ? 'ellip' : ''">
          {{ item.content }}
        </div>
    ...
    <div class="contentList contentList_2 w94" v-else>
      <div class="lengthNo" v-if="communityFriendsList.length == 0">
        该用户暂无社区动态
      </div>
      <div
        class="contentItem"
        v-for="(item, index) in communityFriendsList"
        :key="index"
      >
        <!-- 内容 -->
        <div
        ...
        <div class="timeHandle">
          ...
          <div class="handle">
            ...
            <div
              ...
              @click.stop="showZanAndPinglun(item.id)"
            ></div>
          </div>
        </div>
        <!-- 点赞展示 -->
        ...
        >
          ...
          <div class="zanWord">
            等{{ item.properties.communityLikeList.length }}次赞
          </div>
        </div>
        <!-- 评论展示 -->
        <div
        ...
        >
          ...
          >
            ...
            >
              <span class="s1">{{ comComItem.memberName }}</span
              >回复 <span>{{ comComItem.replyName }}</span
              >：
